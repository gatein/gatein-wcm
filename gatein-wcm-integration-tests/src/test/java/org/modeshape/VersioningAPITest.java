...
public class VersioningAPITest {
    ...
    @Test
    public void createVersioningNodesAndRemovingVersioningDelete() throws Exception {
        ...
        // Adding properties
        n.getNode("jcr:content").setProperty("jcr:data", "This is an UPDATE 1");
        n.setProperty("jcr:description", "This is an UPDATE");
        n.setProperty("jcr:encoding", encoding);

        // Saving changes into JCR
        jcrSession.save();

        jcrSession.getWorkspace().getVersionManager().checkin("/" + id);

//        jcrSession.getWorkspace().getVersionManager().checkout("/" + id);
//
//        // Adding properties
//        n.getNode("jcr:content").setProperty("jcr:data", "This is an UPDATE 2");
//        n.setProperty("jcr:description", "This is an UPDATE");
//        n.setProperty("jcr:encoding", encoding);
//
//        // Saving changes into JCR
...
//        jcrSession.save();
        ...
        jcrSession.getNode("/" + id).removeSharedSet();
        // jcrSession.getNode("/" + id).removeMixin("mix:versionable");

        // jcrSession.removeItem("/" + id);
