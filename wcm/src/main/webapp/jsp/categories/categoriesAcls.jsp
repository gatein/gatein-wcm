...
%>
    <li id="${n}acl<%= acl.getId() %>">
        ...
        <a href="#" onclick="deleteAcl('<%= n %>', '${removeAclCategoryEvent}', '<%= acl.getId() %>', '<%= category.getId() %>')" title="${rsc.getString('categoriesAcls.delete_acl')}"><span class="glyphicon glyphicon-remove wcm-acl-remove"></span></a>
    </li>
<%
        }
    }
