...
%>
    <li id="${n}acl<%= acl.getId() %>">
        ...
        <a href="#" onclick="deleteAcl('<%= n %>', '${removeAclPostEvent}', '<%= acl.getId() %>', '<%= post.getId() %>')" title="${rsc.getString('postsAcls.delete_acl')}"><span class="glyphicon glyphicon-remove wcm-acl-remove"></span></a>
    </li>
<%
        }
    }
