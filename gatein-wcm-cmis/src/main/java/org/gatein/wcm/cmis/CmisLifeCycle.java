...
/**
 * A set of stupid workarounds to make {@link org.modeshape.web.jcr.RepositoryManager} happy.
 *
...
public class CmisLifeCycle {
    ...
    private ModeShapeJcrDeployer modeShapeJcrDeployer;

    public void shutdown() {
        if (modeShapeJcrDeployer != null) {
            ServletContext servletContext = new DummyServletContext();
            servletContext.setInitParameter(RepositoryFactory.URL, "jndi:jcr");
            modeShapeJcrDeployer.contextDestroyed(new ServletContextEvent(servletContext));
        }
    }

    public void startup() {
        modeShapeJcrDeployer = new ModeShapeJcrDeployer();
        ServletContext servletContext = new DummyServletContext();
        servletContext.setInitParameter(RepositoryFactory.URL, "jndi:jcr");
        modeShapeJcrDeployer.contextInitialized(new ServletContextEvent(servletContext));
    }
}
